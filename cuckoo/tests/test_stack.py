...
class FakeCamera:
    ...
    def __init__(
        ...
        subprotocol: str = "secure_transfer",
    ) -> None:
...
def test_ptz_from_the_onvif_side_reaches_the_camera(tmp_path: Path) -> None:
    stack = a_stack(tmp_path)
    camera: FakeCamera | None = None
    ptz_camera: FakeCamera | None = None
    try:
        port = control_port(stack)
        camera = FakeCamera(port)
        camera.send(envelope.HELLO, HELLO_PAYLOAD)
        camera.pump(3.0)
        ptz_camera = FakeCamera(port, subprotocol=ptz.PTZ_SUBPROTOCOL)
        deadline = time.time() + 2.0
        while stack.controller._ptz_session(camera.mac) is None:
            assert time.time() < deadline, "ptz1 channel did not become ready"
            time.sleep(0.05)
        assert stack.controller.cameras[camera.mac].motion.status == "IDLE"
        assert stack.controller._ptz_session(camera.mac) is not None
        before = len([m for m in ptz_camera.received if m.function_name == ptz.PRESET])
        ...
        assert "s:Fault" not in move_response, text_of(move_response, "Text")
        ptz_camera.pump(1.0)
        presets = [m for m in ptz_camera.received if m.function_name == ptz.PRESET]
